...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f'{username} account has been created, enter username and password for authorization')
            return redirect('user')

    else:
        form = UserOurRegistration()
    return render(request, "users/registration.html", {'form': form, 'title': 'Registration User'})
...
def profile(request):
    ...
    return render(request, 'users/profile.html', date)